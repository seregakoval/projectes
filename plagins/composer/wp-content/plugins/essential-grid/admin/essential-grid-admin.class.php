...
class Essential_Grid_Admin extends Essential_Grid_Base {
	...
	public function enqueue_admin_scripts() {
		...
		foreach($post_types as $post_type) {
			...
			if($post_type == $screen->id) wp_enqueue_script('wpdialogs', 'jquery', 'jquery-ui-core', 'jquery-ui-sortable');
		}
	}
		...
		if(!empty($cmetas)){
			foreach($cmetas as $meta){
				if(isset($metas['eg-'.$meta['handle']])){
					if(is_array($metas['eg-'.$meta['handle']])) $metas['eg-'.$meta['handle']] = json_encode($metas['eg-'.$meta['handle']]);
	...
	public static function ess_grid_shortcode_settings_field($settings, $value) {
		...
		return '<div class="ess_grid_shortcode_block">'
