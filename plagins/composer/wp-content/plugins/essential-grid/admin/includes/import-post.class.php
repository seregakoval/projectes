	...
	class PunchPost {
		...
		public function import_taxonomies($terms){
			...
			foreach($terms as $term){		
					if( !term_exists( $term['name'], 'essential_grid_category' ) ){
					wp_insert_term( $term['name'], 'essential_grid_category', array( 'description'	=> $term['category_description'],'slug' => $term['slug'] ) );
				}
			}
		}
