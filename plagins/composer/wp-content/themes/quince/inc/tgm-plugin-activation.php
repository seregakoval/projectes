 ...
 *
 * Note this is a complete fork from the original TGMPA class
 * it won't be able to be merged with the mainstream, and should
 * be updated to the mainstream once they had complete and working
 * plugin upgrading feature.
 *
 * @package   TGM-Plugin-Activation
 * @version   2.5.0-alpha-custom
 * @based     2.5.0-alpha
  ...
  class TGM_Plugin_Activation {
    ...
    public function init() {
      ...
      $this->strings = array(
        ...
        'install_link' => _n_noop('Install plugin', 'Install plugins', 'tgmpa'),
        'update_link' => _n_noop('Update plugin', 'Update plugins', 'tgmpa'),
        'activate_link' => _n_noop('Activate plugin', 'Activate plugins', 'tgmpa'),
        'show_all_link' => _n_noop('List Plugin', 'List Plugins', 'tgmpa'),
    ...
    public function maybe_adjust_source_dir($source, $remote_source, $upgrader) {
      ...
      if (FALSE === $upgrader->bulk) {
        ...
        foreach ($this->plugins as $plugin) {
          if (isset($upgrader->skin->plugin_names[$upgrader->skin->i]) && $plugin['name'] === $upgrader->skin->plugin_names[$upgrader->skin->i]) {
      ...
      if ('' !== $desired_slug) {
        ...
        if (!empty($subdir_name) && $subdir_name !== $desired_slug) {
          ...
          if (TRUE === $GLOBALS['wp_filesystem']->move($from, $to)) {
            ...
            return new WP_Error('rename_failed', esc_html__('The remote plugin package is does not contain a folder with the desired slug and renaming did not work.', 'tgmpa') . ' ' . esc_html__('Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'tgmpa'), array(
              'found' => $subdir_name,
    ...
    public function notices() {
      ...
      if (!empty($message)) {
        ...
        $show_install_link = $install_link ? '<a href="' . esc_url(add_query_arg(array('page' => urlencode($this->menu), 'mode' => 'install'), self_admin_url($this->parent_slug))) . '">' . translate_nooped_plural($this->strings['install_link'], $install_link_count, 'tgmpa') . '</a>' : '';
        $show_activate_link = $activate_link ? '<a href="' . esc_url(add_query_arg(array('page' => urlencode($this->menu), 'mode' => 'activate'), self_admin_url($this->parent_slug))) . '">' . translate_nooped_plural($this->strings['activate_link'], $activate_link_count, 'tgmpa') . '</a>' : '';
        $show_update_link = $update_link ? '<a href="' . esc_url(add_query_arg(array('page' => urlencode($this->menu), 'mode' => 'upgrade'), self_admin_url($this->parent_slug))) . '">' . translate_nooped_plural($this->strings['update_link'], $update_link_count, 'tgmpa') . '</a>' : '';
        $show_all_link = $update_link || $install_link || $activate_link ? '<a href="' . esc_url(add_query_arg('page', urlencode($this->menu), self_admin_url($this->parent_slug))) . '">' . translate_nooped_plural($this->strings['show_all_link'], $update_link_count, 'tgmpa') . '</a>' : '';
        ...
        $action_links = apply_filters(
          ...
          array(
            ...
            //'all' => (current_user_can('activate_plugins') && current_user_can('install_plugins')) ? $show_all_link : '',
...
if (!class_exists('TGMPA_List_Table')) {
  ...
  class TGMPA_List_Table extends WP_List_Table {
    ...
    protected $plugins_install = array();
    protected $plugins_path = array();
    protected $plugins_name = array();
    ...
    protected function _gather_plugin_data() {
      ...
      foreach ($this->tgmpa->plugins as $plugin) {
        ...
        if (((is_plugin_active($plugin['file_path']) || (isset($plugin['is_callable']) && is_callable($plugin['is_callable']))) && !$do_update)
          || (self::$mode == 'upgrade' && !$do_update)
          || (self::$mode == 'install' && $do_update)) {
    ...
    public function get_bulk_actions() {
      ...
      if (self::$mode == 'install') {
        unset($actions['tgmpa-bulk-update']);
      }
      if (self::$mode == 'upgrade') {
        unset($actions['tgmpa-bulk-install']);
        unset($actions['tgmpa-bulk-activate']);
      }
    ...
    /**
     * Method for preprocessing plugins
     * variables. Call this before the
     * do_upgrade and do_install method.
     */
    protected function prepare_plugins() {
      ...
      if ( isset( $_GET['plugins'] ) ) {
        $this->plugins = explode( ',', stripslashes( $_GET['plugins'] ) );
      }
      // Looks like the user can use the direct method, take from $_POST.
      elseif ( isset( $_POST['plugin'] ) ) {
        $this->plugins = (array) $_POST['plugin'];
      }

      // Grab information from $_POST if available.
      if ( isset( $_POST['plugin'] ) ) {
        foreach ( $this->plugins as $plugin_data ) {
          $this->plugins_to_install[] = explode( ',', $plugin_data );
        }

        foreach ( $this->plugins_to_install as $plugin_data ) {
          $this->plugins_install[] = $plugin_data[0];
          $this->plugins_path[]    = $plugin_data[1];
          $this->plugins_name[]    = $plugin_data[2];
        }
      }
      // Information has been passed via $_GET.
        ...
        foreach ( $this->plugins as $key => $value ) {
          ...
          if ( 0 === ( $key % 3 ) || 0 === $key ) {
            $this->plugins_to_install[] = $value;
            $this->plugins_install[]    = $value;
          }
        }
      }
      ...
      if ( isset( $_GET['plugin_paths'] ) ) {
        $this->plugins_path = explode( ',', stripslashes( $_GET['plugin_paths'] ) );
      }
      // Looks like the user doesn't need to enter his FTP credentials.
      elseif ( isset( $_POST['plugin'] ) ) {
        $this->plugins_path = (array) $this->plugins_path;
      }
      // Nothing has been submitted.
      else {
        $this->plugins_path = array();
      }

      // Look first to see if information has been passed via WP_Filesystem.
      if ( isset( $_GET['plugin_names'] ) ) {
        $this->plugins_name = explode( ',', stripslashes( $_GET['plugin_names'] ) );
      }
      // Looks like the user doesn't need to enter his FTP credentials.
      elseif ( isset( $_POST['plugin'] ) ) {
        $this->plugins_name = (array) $this->plugins_name;
      }
      // Nothing has been submitted.
      else {
      ...
      $i = 0;
      foreach ( $this->plugins_install as $key => $plugin ) {
        if ( preg_match( '|.php$|', $this->plugin ) ) {
          unset( $this->plugins_install[ $key ] );

          // If the plugin path isn't in the $_GET variable, we can unset the corresponding path.
          if ( ! isset( $_GET['plugin_paths'] ) ) {
            unset( $this->plugins_path[ $i ] );
          }

          // If the plugin name isn't in the $_GET variable, we can unset the corresponding name.
          if ( ! isset( $_GET['plugin_names'] ) ) {
            unset( $this->plugins_name[ $i ] );
          }
        }
        $i++;
      }

      if (!empty($this->plugins_install)) {
        // Reset array indexes in case we removed already installed plugins.
        $this->plugins_install = array_values($this->plugins_install);
        $this->plugins_path = array_values($this->plugins_path);
        $this->plugins_name = array_values($this->plugins_name);

        // If we grabbed our plugin info from $_GET, we need to decode it for use.
        $this->plugins_install = array_map('urldecode', $this->plugins_install);
        $this->plugins_path = array_map('urldecode', $this->plugins_path);
        $this->plugins_name = array_map('urldecode', $this->plugins_name);
      }
    ...
    protected function process_installation() {
      ...
      if (!WP_Filesystem($creds)) {
        request_filesystem_credentials($this->url, $method, TRUE, FALSE, $fields); // Setup WP_Filesystem.
        return TRUE;
      }
    ...
    /**
     * Method for the actual processing when on bulk
     * install mode.
     * @return bool
     */
    protected function do_bulk_install() {
      ...
      $this->url = wp_nonce_url(
        add_query_arg(
          array(
            ...
            'plugins' => urlencode(implode(',', $this->plugins)),
            'plugin_paths' => urlencode(implode(',', $this->plugins_path)),
            'plugin_names' => urlencode(implode(',', $this->plugins_name)),
          ),
          self_admin_url($this->tgmpa->parent_slug)
        ),
        'bulk-plugins'
      ...
      return $this->process_installation();
    }
     ...
     * @return bool
     */
    protected function do_bulk_upgrade() {

      // No need to proceed further if we have no plugins to install.
      if (empty($this->plugins_install)) {
        echo '<div id="message" class="error"><p>' . esc_html(__('No plugins are available to be updated at this time.', 'tgmpa')) . '</p></div>';
        return FALSE;
      }
     ...
     * @return bool
     */
    protected function do_bulk_activate() {
      check_admin_referer('bulk-' . $this->_args['plural']);
    ...
    public function process_bulk_actions() {
      ...
      if ('tgmpa-bulk-install' === $this->current_action()) {
        ...
        $result = $this->do_bulk_install();
      }

      // Bulk activation process.
      if ('tgmpa-bulk-activate' === $this->current_action()) {
        $this->do_bulk_activate();
      }

      // Bulk update process.
      if ('tgmpa-bulk-update' === $this->current_action()) {
        $this->prepare_plugins();
        $result = $this->do_bulk_upgrade();
      }
    ...
    public function prepare_items() {
      ...
      $this->items = apply_filters('plugin_table_items', $this->_gather_plugin_data());
      ...
      if (!class_exists('TGM_Bulk_Installer')) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            foreach ($packages as $plugin) {
              ...
              $result = $this->run(
                array(
                  ...
                  // Do we want to clear the destination or not?
                  'clear_working' => TRUE,
                  // Remove original install file.
                  'is_multi' => TRUE,
                  // Are we processing multiple installs?
                  'hook_extra' => array(
                    'plugin' => $plugin,
                    'update' => $update
                  ),
                  // Pass plugin source as extra data.
                )
              );
          ...
          public function install_strings() {
            ...
            $this->strings['remove_old_failed'] = __('Failed to remove old plugin directory', 'tgmpa');
      ...
      if (!class_exists('TGM_Bulk_Installer_Skin')) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
          ...
          public function __construct($args = array()) {
            ...
            // Set plugin names to $this->plugins_name property.
            $this->plugins_name = $args['names'];
          ...
          public function before($title = '') {
            ...
            printf('<h4>' . wp_kses_post($this->upgrader->strings['skin_before_update_header']) . ' <img alt="" src="' . esc_url(admin_url('images/wpspin_light.gif')) . '" class="hidden waiting-' . esc_attr($this->upgrader->update_current) . '" style="vertical-align:middle;" /></h4>', esc_html($this->plugins_name[$this->i]), absint($this->upgrader->update_current), absint($this->upgrader->update_count));
          ...
          public function after($title = '') {
            ...
            if ($this->error || !$this->result) {
              if ($this->error) {
                echo '<div class="error"><p>', sprintf(wp_kses_post($this->upgrader->strings['skin_update_failed_error']), esc_html($this->plugins_name[$this->i]), wp_kses_post($this->error)), '</p></div>';
              }
              else {
                echo '<div class="error"><p>', sprintf(wp_kses_post($this->upgrader->strings['skin_update_failed']), esc_html($this->plugins_name[$this->i])), '</p></div>';
              }
            ...
            if (!empty($this->result) && !is_wp_error($this->result)) {
              echo '<div class="updated"><p>', sprintf($this->upgrader->strings['skin_update_successful'] /* pre-escaped */, esc_html($this->plugins_name[$this->i]), 'jQuery(\'#progress-' . esc_js($this->upgrader->update_current) . '\').toggle();jQuery(\'span\', this).toggle(); return false;'), '</p></div>';
