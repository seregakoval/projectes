...
function mnky_breadcrumb( $args = '' ) {
		...
		$separator = ( !empty( $args['separator'] ) ) ? "<span class='breadcrumbs-separator'>{$args['separator']}</span>" : "<span class='separator'>/</span>";

		/* Get the items. */
		$items = mnky_breadcrumb_get_items( $args );

		$breadcrumbs = '<nav class="breadcrumbs-trail" role="navigation">';
	...
	function mnky_breadcrumb_get_items( $args ) {
		...
		elseif ( is_singular() ) {
			...
			if ( 'page' !== $wp_query->post->post_type ) {
				...
				if ( function_exists( 'get_post_type_archive_link' ) && !empty( $post_type_object->has_archive ) )
					$item[] = '<a href="' . esc_url(get_post_type_archive_link( $post_type )) . '" title="' . esc_attr( $post_type_object->labels->name ) . '">' . $post_type_object->labels->name . '</a>';
		...
		/* If viewing a 404 error page. */
			...
			$item['last'] = __( 'Page Not Found', 'quince' );
