...
function ResponsivePhotoGallery_init() {
	...
	add_meta_box('Pro Features', 'Pro Features', 'wrg_pro_features', 'responsive-gallery', 'side', 'low');
    ...
    if($TotalImages) {
        foreach($rpg_all_photos_details as $rpg_single_photos_detail) {
			...
			<div class="rpg-image-entry" id="rpg_img<?php echo $i; ?>">
				...
				<input type="text" id="rpg_img_url<?php echo $i; ?>" name="rpg_img_url<?php echo $i; ?>" class="rpg_label_text" value="<?php echo esc_url($url); ?>" readonly="readonly" style="display:none;" />
				<input type="text" id="image_label<?php echo $i; ?>" name="image_label<?php echo $i; ?>" placeholder="Enter Image Label" class="rpg_label_text" value="<?php echo esc_html($name); ?>">
			</div>
            <?php
            $i++;
        } // end of foreach
...
function WRG_SettingsPage() {
    ...
    add_submenu_page('edit.php?post_type=responsive-gallery', 'recommendation', 'Plugin Recommendation', 'administrator', 'plugin-recommendation', 'RPG_plugin_recommendation');
}
