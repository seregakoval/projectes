 ...
 * @since 1.5
 */
class plgEditorTinymce3 extends JPlugin
{
	/**
	 * Constructor
	 *
	 ...
	 * @since 1.5
	 */
	function plgEditorTinymce3(& $subject, $config)
	{
	...
	function onInit()
	{
		...
		$load = "\t<script type=\"text/javascript\" src=\"".JURI::root()."plugins/editors/tinymce3/jscripts/tiny_mce/tiny_mce.js\"></script>\n";
...
<script type="text/javascript">
	tinyMCE.init({
		...
		//Плагины
		plugins : "safari,pagebreak,style,layer,table,save,advhr,advimage,advlink,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,template,compat2x",
...
EOF;
	...
	function onGetContent( $editor ) {
		return "tinyMCE.get('tinyeditor3').getContent();";
	}
	...
	function onSetContent( $editor, $html ) {
		return "tinyMCE.get('tinyeditor3').setContent($html);";
	}
	...
	function onSave( $editor ) {
		return "tinyMCE.get('tinyeditor3').save();";
	}
	...
	function onDisplay( $name, $content, $width, $height, $col, $row, $buttons = true)
	{
		...
		$buttons = $this->_displayButtons('tinyeditor3', $buttons);
		$editor  = "<textarea id=\"tinyeditor3\" name=\"$name\" cols=\"$col\" rows=\"$row\" style=\"width:{$width}; height:{$height};\" class=\"mce_editable\">$content</textarea>\n" . $buttons;
		//class=\"mce_editable=\"
