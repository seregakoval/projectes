<?php 
/**
 * $Id: admin.xmap.html.php 43 2009-08-01 19:21:35Z guilleva $
...
<?php
	...
	function showNavigatorLinks($sitemapid,$list,$name)
	{
		...
		foreach ($list as $node) {
			...
			echo "<node text=\"".htmlentities($node->name)."\" ".($node->expandible?" openicon=\"_open\" icon=\"_closed\" load=\"$load\"":' icon="_doc"')." uid=\"".$node->uid."\" link=\"".str_replace(array('&amp;','&'),array('&','&amp;'),$node->link)."\" selectable=\"".($node->selectable?'true':'false')."\">\n";
			echo "</node>\n";
		}
		echo "</nodes>";
		exit;
