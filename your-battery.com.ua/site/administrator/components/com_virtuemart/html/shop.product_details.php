...
$flypage = vmGet($_REQUEST, "flypage" );
        ...
        //Pusk i emkost`
        if ($db_product->f("product_capacity")>0) {
            $ps_cap = $db_product->f("product_capacity");
            $ps_cap = explode('.',$ps_cap);
            
            if ($ps_cap[1]>0) {
                $ps_cp = $VM_LANG->_('PHPSHOP_PRODUCT_FORM_CAPACITY').':&nbsp;'.$ps_cap[0].",".$ps_cap[1].$VM_LANG->_('PHPSHOP_PRODUCT_FORM_CAPACITY_AH');
            }
            else{
                $ps_cp = $VM_LANG->_('PHPSHOP_PRODUCT_FORM_CAPACITY').':&nbsp;'.$ps_cap[0].$VM_LANG->_('PHPSHOP_PRODUCT_FORM_CAPACITY_AH');
            }  
        }
        
        if ($db_product->f("product_starting")>0) {
                $ps_cp .= '<br/>'.$VM_LANG->_('PHPSHOP_PRODUCT_FORM_STARTING').':&nbsp;'.$db_product->f("product_starting").$VM_LANG->_('PHPSHOP_PRODUCT_FORM_STARTING_A');
            }  
        
        
        $product_starting_capacity = "<span class='browse_product_capacity'>".$ps_cp."</span>";
        //Pusk i emkost`
        
        
        if ($db_product->f("product_voltage")>0) {
        $ps_voltage = $VM_LANG->_('PHPSHOP_PRODUCT_FORM_VOLTAGE').':&nbsp;'.$db_product->f("product_voltage").$VM_LANG->_('PHPSHOP_PRODUCT_FORM_VOLTAGE_V').'<br/>';
        }
        else {
            $ps_voltage ='';
        }
        ...
        //Gabaritu
        if ($db_product->f("product_length")>0 || $db_product->f("product_width")>0 || $db_product->f("product_height")>0) {
            $ps_lwh = $VM_LANG->_('PHPSHOP_PRODUCT_FORM_LWH').':&nbsp;'.$db_product->f("product_length").'/'.$db_product->f("product_width").'/'.$db_product->f("product_height");
        }
        $product_lwh = "<br/><span class='browse_product_lwh'>".$ps_lwh."</span>";

        //Gabaritu


        //Otobragenie kartinki pol`iarnosti
        if (strstr($product_name, ' '.$VM_LANG->_('PHPSHOP_POLIAR_0'))) {
            $url_0=VM_THEMEURL . 'images/p0.png';
            $orientir='<img src="'.$url_0.'" width="93px" height="67px" >';
        }
        Else if (strstr($product_name, ' '.$VM_LANG->_('PHPSHOP_POLIAR_1'))) {
            $url_1=VM_THEMEURL . 'images/p1.png';
            $orientir='<img src="'.$url_1.'" width="93px" height="67px">';
            }
        //Otobragenie kartinki pol`iarnosti
...
//Pusk tok i gabaritu
$tpl->set( 'product_starting_capacity', $product_starting_capacity );
$tpl->set( 'product_lwh', $product_lwh );
//Pusk tok i gabaritu
...
$tpl->set( "orientir", $orientir );
$tpl->set( "ps_voltage", $ps_voltage );
