...
elseif( $num_rows == 1 && ( !empty($keyword) || !empty($keyword1) ) ) {
	...
	while ($db_browse->next_record()) {
        ...
        //Pusk i emkost`
        if ($db_browse->f("product_capacity")>0) {
            $ps_cap = $db_browse->f("product_capacity");
        ...
        if ($db_browse->f("product_starting")>0) {
            $ps_cp .= '<br/>'.$VM_LANG->_('PHPSHOP_PRODUCT_FORM_STARTING').':&nbsp;'.$db_browse->f("product_starting").$VM_LANG->_('PHPSHOP_PRODUCT_FORM_STARTING_A');
        }
        
          
        
        if ($db_browse->f("product_voltage")>0) {
        $ps_voltage = $VM_LANG->_('PHPSHOP_PRODUCT_FORM_VOLTAGE').':&nbsp;'.$db_browse->f("product_voltage").$VM_LANG->_('PHPSHOP_PRODUCT_FORM_VOLTAGE_V').'<br/>';
        }
        else {
            unset ($ps_voltage);
        }
        ...
        //Gabaritu
        if ($db_browse->f("product_length")>0 || $db_browse->f("product_width")>0 || $db_browse->f("product_height")>0) {
            $ps_lwh = $VM_LANG->_('PHPSHOP_PRODUCT_FORM_LWH').':&nbsp;'.$db_browse->f("product_length").'/'.$db_browse->f("product_width").'/'.$db_browse->f("product_height");
        ...
        $products[$i]['product_lwh'] = $product_lwh;
        $products[$i]['product_starting_capacity'] = $product_starting_capacity;
        
        $products[$i]['ps_voltage'] = $ps_voltage;   
        
        $products[$i]['orientir'] = $orientir;
