...
*
* @version $Id: shop_browse_queries.php 1879 2009-09-11 08:20:46Z soeren_nb $
* @package VirtueMart
* @subpackage html
...
$fieldnames = "`product_name`,`products_per_row`,`category_browsepage`, `product_capacity`,`product_voltage`,`product_starting`,`category_flypage`,`#__{vm}_category`.`category_id`,
...
switch( $orderby ) {
    ...
    case 'product_starting':
		$orderbyField = '`#__{vm}_product`.`product_starting`'; break;
    case 'product_capacity':
		$orderbyField = '`#__{vm}_product`.`product_capacity`'; break;
    case 'product_voltage':
		$orderbyField = '`#__{vm}_product`.`product_voltage`'; break;
	case 'product_price':
...
elseif( !empty($keyword1Arr) ) {
	...
	foreach( $keyword1Arr as $searchstring ) {
		...
		default:
            ...
            $sq .= "\n `#__{vm}_product`.`product_capacity` LIKE '%$searchstring%' OR ";
            $sq .= "\n `#__{vm}_product`.`product_starting` LIKE '%$searchstring%' OR ";
            $sq .= "\n `#__{vm}_product`.`product_voltage` LIKE '%$searchstring%' OR ";
			$sq .= "\n `#__{vm}_product`.`product_sku` LIKE '%$searchstring%' OR ";
			$sq .= "\n `#__{vm}_product`.`product_s_desc` LIKE '%$searchstring%' OR ";
			$sq .= "\n `#__{vm}_product`.`product_desc` LIKE '%$searchstring%') ";
		}
