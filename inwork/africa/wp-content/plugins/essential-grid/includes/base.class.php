...
class Essential_Grid_Base {
	...
	public static function getPostsByCategory($grid_id, $catID, $postTypes="any", $taxonomies="category", $pages = array(), $sortBy = 'ID', $direction = 'DESC', $numPosts=-1, $arrAddition = array(), $enable_caching = true, $relation = 'OR'){ //category
		...
		if($enable_caching){
			//add option to disable caching fully
			$enable_caching = apply_filters('essgrid_query_caching', $enable_caching, $grid_id);
			if($enable_caching){
		...
		$arrPosts = apply_filters('essgrid_modify_posts', $arrPosts, $grid_id);
			...
			case 'filter':
				...
				if(!empty($tags)){
					foreach($tags as $key => $tag){
						...
						$text .= '<a href="#" class="eg-triggerfilter" data-filter="filter-'.$tag.'">'.$tag.'</a>';
					}
				}
				return $text;
			break;
