<?php
if (!class_exists('OP_CurlResponse')) require_once('curl_response.php');
 ...
 * @version $id$
 */
interface OP_OAuthServiceProvider {
 ...
 * @version $id$
 */
class OP_OAuthApplication implements OP_AWeberOAuthAdapter {
    public $debug = false;
    ...
    public function __construct($parentApp = false) {
        if ($parentApp) {
            if (!is_a($parentApp, 'OP_OAuthServiceProvider')) {
                throw new Exception('Parent App must be a valid OAuthServiceProvider!');
            }
            $this->app = $parentApp;
        }
        $this->user = new OP_OAuthUser();
    }
    ...
    public function getRequestToken($callbackUrl=false) {
        ...
        $response = $this->makeRequest('POST', $this->app->getRequestTokenUrl(), $data);
    ...
    public function getAccessToken() {
        $response = $this->makeRequest('POST', $this->app->getAccessTokenUrl(),
        ...
        if (empty($data['oauth_token'])) {
            throw new OP_AWeberOAuthDataMissing('oauth_token');
        }
    ...
    public function parseAsError($response) {
        if (!empty($response['error'])) {
            throw new OP_AWeberOAuthException($response['error']['type'],
    ...
    protected function requiredFromResponse($data, $requiredFields) {
        foreach ($requiredFields as $field) {
            if (empty($data[$field])) {
                throw new OP_AWeberOAuthDataMissing($field);
            }
        }
    }
    ...
    public function makeRequest($method, $url, $data=array()) {
        ...
        if (!$resp) {
            ...
            throw new OP_AWeberAPIException($error, $url);
        }

        if($resp->headers['Status-Code'] >= 400 && $resp->headers['Status-Code'] != 403) {
            $data = json_decode($resp->body, true);
            throw new OP_AWeberAPIException($data['error'], $url);
        }
    ...
    private function _sendRequest($handle, $headers = array('Expect:')) {
        ...
        if ($resp) {
            return new OP_CurlResponse($resp);
        }
