 ...
 *
 */
class OP_CS_REST_Wrapper_Result {
    /**
    ...
    function OP_CS_REST_Wrapper_Result($response, $code) {
...
class OP_CS_REST_Wrapper_Base {
     ...
     * @var OP_CS_REST_JsonSerialiser or OP_CS_REST_XmlSerialiser
     * @access private
     */
    var $_serialiser;
     ...
     * @var OP_CS_REST_CurlTransport or OP_CS_REST_SocketTransport or your own custom transport.
     * @access private
     */
    var $_transport;
    ...
    function OP_CS_REST_Wrapper_Base(
        ...
        $this->_log = is_null($log) ? new OP_CS_REST_Log($debug_level) : $log;
        ...
        $this->_transport = is_null($transport) ?
            OP_CS_REST_TRANSPORT_get_available($this->is_secure(), $this->_log) :
            $transport;
        ...
        $this->_serialiser = is_null($serialiser) ?
            OP_CS_REST_SERIALISATION_get_available($this->_log) : $serialiser;
    ...
    function set_logger(OptimizePress_Modules_Email_LoggerInterface $logger)
    {
        $this->logger = $logger;
    }
