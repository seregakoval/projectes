...
function OP_CS_REST_SERIALISATION_get_available($log) { 
    $log->log_message('Getting serialiser', __FUNCTION__, CS_REST_LOG_VERBOSE);
    if(function_exists('json_decode') && function_exists('json_encode')) {
        return new OP_CS_REST_NativeJsonSerialiser($log);
    } else {
        return new OP_CS_REST_ServicesJsonSerialiser($log);
    }    
}
class OP_CS_REST_BaseSerialiser {

    var $_log;
    
    function OP_CS_REST_BaseSerialiser($log) { 
        $this->_log = $log;
    }
...
class OP_CS_REST_DoNothingSerialiser extends OP_CS_REST_BaseSerialiser {
    function OP_CS_REST_DoNothingSerialiser() {}
...
class OP_CS_REST_NativeJsonSerialiser extends OP_CS_REST_BaseSerialiser {

    function OP_CS_REST_NativeJsonSerialiser($log) {
        $this->OP_CS_REST_BaseSerialiser($log);
    }
...
class OP_CS_REST_ServicesJsonSerialiser extends OP_CS_REST_BaseSerialiser {
    
    var $_serialiser;
    
    function OP_CS_REST_ServicesJsonSerialiser($log) {
        $this->OP_CS_REST_BaseSerialiser($log);
        $this->_serialiser = new OP_Services_JSON();
    }
