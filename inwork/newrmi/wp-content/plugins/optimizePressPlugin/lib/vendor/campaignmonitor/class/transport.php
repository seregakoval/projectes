...
function OP_CS_REST_TRANSPORT_get_available($requires_ssl, $log) {
    if(function_exists('curl_init') && function_exists('curl_exec')) {
        return new OP_CS_REST_CurlTransport($log);
    } else if(OP_CS_REST_TRANSPORT_can_use_raw_socket($requires_ssl)) {
        return new OP_CS_REST_SocketTransport($log);
    } else { 
...
function OP_CS_REST_TRANSPORT_can_use_raw_socket($requires_ssl) {
...
class OP_CS_REST_BaseTransport {
    
    var $_log;
    
    function OP_CS_REST_BaseTransport($log) {
        $this->_log = $log;
    }
 ...
 * @since 1.0
 */
class OP_CS_REST_CurlTransport extends OP_CS_REST_BaseTransport {

    var $_curl_zlib;

    function OP_CS_REST_CurlTransport($log) {
        $this->OP_CS_REST_BaseTransport($log);
...
class OP_CS_REST_SocketWrapper {
    var $socket;
...
class OP_CS_REST_SocketTransport extends OP_CS_REST_BaseTransport {

    var $_socket_wrapper;

    function OP_CS_REST_SocketTransport($log, $socket_wrapper = NULL) {
        $this->OP_CS_REST_BaseTransport($log);

        if(is_null($socket_wrapper)) {
            $socket_wrapper = new OP_CS_REST_SocketWrapper();
        }
