<div id="op-revisions-dialog" class="op-revisions-dialog">
    <h1><?php _e('Page Revisions',OP_SN) ?></h1>
    <div class="op-lightbox-content">
        ...
        <?php if (!empty($revisions)) : ?>
            <div class="op-warning-message">
                <strong><?php _e('Please note:', OP_SN); ?></strong> <?php _e('The OptimizePress Revisions system will autosave pages every 5 minutes and store 10 page revisions.  Revisions of only your main page content is saved, for other settings including Typography, Layout and Page settings the most recent version will always be used.', OP_SN); ?>
            </div>
            ...
            <?php foreach ($revisions as $revision) : ?>
                <li class="op-revisions-list-item">
                    ...
                    <label class="op-revisions-list-label">
                        ...
                        <?php echo __('Revision from ', OP_SN) . date('d.m.Y H:i:s', strtotime($revision->modified)) ?> -
                    </label>
                    <a style="color:inherit; text-decoration: underline;" class="op-revision-restore" href="#1" data-revisionid="<?php echo $revision->id; ?>" data-postid="<?php echo $revisions_page_id ?>"><?php _e('Restore', OP_SN); ?></a>
                </li>
            <?php endforeach; ?>
            </ul>
            <div class="op-revisions-diff">
                <table class="op-diff">
                    <thead>
                        <tr>
                            <th class="op-diff-th"><?php _e('Current state', OP_SN); ?></th>
                            <th>&nbsp;</th>
                            <th class="op-diff-th">
                                <?php _e('Revision', OP_SN); ?>
                                <a target="_blank" id="op-open-revision-new-tab" class="op-open-revision-new-tab">(<?php _e('Open in New Tab', OP_SN); ?>)</a>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
