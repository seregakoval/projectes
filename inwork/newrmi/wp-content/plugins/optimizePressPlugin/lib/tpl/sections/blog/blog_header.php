<div class="op-bsw-grey-panel-content op-bsw-grey-panel-no-sidebar cf">
    ...
    ?>
    <label for="op_sections_blog_header_logo" class="form-title"><?php _e('Upload a logo (optional)',OP_SN) ?></label>
    <p class="op-micro-copy"><?php _e('Upload a logo image to show on your blog. We recommend you size your logo to '.OP_HEADER_LOGO_WIDTH.'px by '.OP_HEADER_LOGO_HEIGHT.'px',OP_SN) ?></p>
   ...
   <!-- <div class="op-hr"><hr /></div> -->
    ...
    <label for="op_sections_blog_header_bgimg" class="form-title"><?php _e('Upload a Banner Image',OP_SN) ?></label>
    <p class="op-micro-copy"><?php _e('Recommended if you don\'t have a logo. Upload a header image up to 975px in width with any graphics on it, and we\'ll throw that on instead.',OP_SN) ?></p>
    <?php op_upload_field('op[sections][blog_header][bgimg]',op_default_option('blog_header','bgimg')) ?>
    
    
    <label for="op_sections_blog_header_repeatbgimg" class="form-title"><?php _e('Upload Repeating Header Background Image',OP_SN) ?></label>
