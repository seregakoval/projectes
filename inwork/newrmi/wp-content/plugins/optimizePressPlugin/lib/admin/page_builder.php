...
class OptimizePress_PageBuilder {
	...
	function get_step(){
		if(!isset($this->cur_step)){
			...
			if($this->cur_step > 0){
				...
				$sections = array(
					1 => __('Proceed to Step 2', OP_SN),
					2 => __('Proceed to Step 3', OP_SN),
					3 => __('Proceed to Step 4', OP_SN),
					4 => __('Save Settings &amp; Launch LiveEditor', OP_SN),
				);
	...
	function admin_menu(){
		$page = add_submenu_page(OP_SN, __('Page Builder',OP_SN), __('Page Builder',OP_SN), 'edit_pages', OP_SN.'-page-builder', array($this,'page_builder'));
	...
	function print_scripts(){
		if(OP_CONTENT_UPLOAD){
			...
			wp_enqueue_script( OP_SN.'-admin-page-wizard',  OP_JS.'page_builder'.OP_SCRIPT_DEBUG.'.js', array(OP_SN.'-noconflict-js', OP_SN.'-admin-common'), OP_VERSION);
		}
	}
	...
	function save_setup_wizard(){
		...
		if(OP_REINIT_PAGE_THEME && OP_SHOW_THEME_MSG){
			$this->notification = __('Your theme has been changed. Please review ALL settings',OP_SN);
		}
	}
	...
	function setup_wizard(){
		...
		if($this->cur_step > 5){
			return op_show_error(__('You have completed the wizard.', OP_SN));
		}
	...
	function step_1(){
		global $wpdb;
		$this->js['slug_message'] = __('Please provide a valid URL slug which is not in use.',OP_SN);
		$this->js['name_message'] = __('Please provide a name for your page.',OP_SN);
		$permalinks = true;
		...
		$presets = array(
			...
			'content_layout' => __('Use a Content Template',OP_SN),
			'preset' => __('Use a Saved Preset',OP_SN),
		);
		if(defined('OP_PAGEBUILDER_ID')){
			$blankimg = 'pb_page_blank.png';
			$presets['blank'] = __('Use Current Settings',OP_SN);
	...
	function save_step_1(){
		...
		if(count($op) > 0 && isset($op['name'])){
			$post_arr = array('post_title'=>op_get_var($op,'name'),'post_name'=>op_get_var($op,'slug'),'post_type'=>'page');
			if(!defined('OP_PAGEBUILDER_ID')){
	...
	function step_2(){
		...
		$types = array(
			'landing' => __('Landing Page',OP_SN),
			'marketing' => __('Marketing Site Page',OP_SN),
			//'launch' => __('Launch Funnel Page',OP_SN),
			'membership' => __('Membership Page',OP_SN),
		);
	...
	function _sections(){
		$sections = array(
			'layout' => array(
				'title' => __('Layout', OP_SN),
				'description' => __('Customize the layout of your template. Setup your headers, navigation bars, feature areas and footers.', OP_SN),
			),
			'color_schemes' => array(
				'title' => __('Colour Scheme', OP_SN),
				'description' => __('Customize the colour scheme for your page. You can use the advanced section to customize individual element colours', OP_SN),
			),
			'functionality' => array(
				'title' => __('On-Page Functionality', OP_SN),
				'description' => __('Control the additional functionality and options for your page below. You can also use the options below to add any custom script code to your page', OP_SN),
			),
		);
		$i = 1;
		foreach($sections as $name => $section){
			if(!(op_page_config('disable',$name) === true)){
				...
				$this->sections[$name] = array(
					'object' => new $class(),
					'title' => ''.__($section['title'],OP_SN),
					'description' => __($section['description'],OP_SN)
				);
				$i++;
			}
		}
