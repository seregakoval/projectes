...
class OptimizePress_ThemeSettings {
	...
	function enable_blog(){
		if(isset($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],'op_theme_settings')){
			...
			$arr = array('error' => __('Verification failed, please refresh the page and try again.',OP_SN));
		}
	...
	function get_sections(){
		if(!isset($this->sections)){
			...
			$this->sections = array();
			$sections = array(
				'brand' => __('Brand', OP_SN),
				'layout' => __('Layout Structure', OP_SN),
				'modules' => __('Modules', OP_SN),
			);
	...
	function admin_menu(){
		$page = add_submenu_page(OP_SN, __('Blog Settings',OP_SN), __('Blog Settings',OP_SN), 'edit_theme_options', OP_SN.'-theme-settings', array($this,'theme_settings'));
