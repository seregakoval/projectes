...
class OptimizePress_Dashboard {
	...
	function get_sections(){
		...
		if(!isset($this->sections)){
			...
			$sections = array(
				'global_settings' => __('Global Settings', OP_SN),
				'analytics_and_tracking' => __('Analytics and Tracking', OP_SN),
				'email_marketing_services' => __('Email Marketing Services', OP_SN),
				'social_integration' => __('Social Integration', OP_SN),
			);
	...
	function admin_menu(){
		...
		//Add the submenu
		$page = add_submenu_page(OP_SN, __('Dashboard',OP_SN), __('Dashboard',OP_SN), 'edit_theme_options', OP_SN, array($this,'dashboard'));
	...
	function print_scripts(){
		...
		wp_enqueue_script( OP_SN.'-admin-common', false, array(OP_SN.'-noconflict-js'), OP_VERSION);
	}
	...
	function save_step(){
		...
		if(op_has_error())
			...
			$this->notification = __('Your settings have been updated.',OP_SN);
	}
}
