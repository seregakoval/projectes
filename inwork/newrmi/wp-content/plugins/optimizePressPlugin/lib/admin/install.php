...
class OptimizePress_Install {
	...
	function install_screen(){
		...
		if (false === op_sl_get_key()) $this->error = __('API key missing or invalid', OP_SN);
		...
		if(isset($_POST[OP_SN.'_install'])){
			...
			if(isset($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],'op_install')){
				...
				if (is_wp_error($status)) {
					$this->error = __('API key is invalid. Please re-check it.', OP_SN);
				} else {
					//Save the API key
					op_sl_save_key($key);
					...
					foreach($layouts as $layout_name => $layout){
						...
						foreach($layout as $row){
							...
							if (!empty($row['row_data_style'])) {
								...
								//error_log('beforent: ' . print_r($replace, true));
								$temp = str_replace($find, $replace, $temp);
								//error_log('after: ' . $temp);
