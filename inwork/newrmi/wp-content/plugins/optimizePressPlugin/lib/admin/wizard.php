...
class OptimizePress_SetupWizard {
	...
	function get_step(){
		if(!isset($this->cur_step)){
			...
			if($this->cur_step > 0){
				$sections = array(
					1 => array('submit_button' => __('Continue to Step 2: Brand Your Blog', OP_SN)),
					2 => array('section' => 'brand','submit_button' => __('Continue to Step 3: Choose Layout', OP_SN)),
					3 => array('section' => 'layout','submit_button' => __('Continue to Step 4: Blog Modules', OP_SN)),
					4 => array('section' => 'modules','submit_button' => __('Continue to Step 5: Finish Setup', OP_SN)),
				);
	...
	function admin_menu(){
		$page = add_submenu_page(OP_SN, __('Blog Setup',OP_SN), __('Blog Setup',OP_SN), 'edit_theme_options', OP_SN.'-setup-wizard', array($this,'setup_wizard'));
	...
	function print_scripts(){
		...
		wp_enqueue_script(OP_SN.'-admin-common', false, array(OP_SN.'-noconflict-js'), OP_VERSION); //'-blog-settings');
	}
