...
class OptimizePress_LaunchSuite {
	...
	function create_funnel(){
		...
		$arr = array(
			'error' => __('Verification failed, please refresh the page and try again.',OP_SN)
		);
		...
		if(wp_verify_nonce($nonce,$nonce_chk)){
			...
			$arr['error'] = __('Please provide a name for your funnel',OP_SN);
			if(!empty($name)){
				$funnel_dropdown = '';
	...
	function refresh_dropdown(){
		...
		if(wp_verify_nonce($nonce,'op_launch_suite')){
			...
			$arr['error'] = __('No funnel ID was provided',OP_SN);
			if(!empty($id)){
	...
	function admin_menu(){
		$page = add_submenu_page(OP_SN, __('Launch Suite',OP_SN), __('Launch Suite',OP_SN), 'edit_theme_options', OP_SN.'-launch-suite', array($this,'launch_suite'));
	...
	function print_scripts(){
		...
		wp_enqueue_script(OP_SN.'-admin-launch-suite',  OP_JS.'launch_suite'.OP_SCRIPT_DEBUG.'.js', array(OP_SN.'-noconflict-js', OP_SN.'-admin-common','jquery-ui-sortable'), OP_VERSION);
	}
	...
	function save_launch_suite(){
		...
		$this->data['settings_sections'] = array(
			'gateway_key' => __('Funnel Gateway Key',OP_SN),
			'perpetual_launch' => __('Perpetual/Evergreen Launch Mode',OP_SN),
			'redirect_all' => __('Redirect All Launch Pages',OP_SN),
			'hide_coming_soon' => __('Hide Coming Soon Placeholders',OP_SN),
		);
	...
	function _generate_funnel_pages(){
		...
		$tabs = array(
			'page_setup_sales' => __('Page Setup',OP_SN),
			'page_thumbnails' => __('Page Thumbnails',OP_SN),
			'navigation_text' => __('Navigation Text',OP_SN),
		);
		...
		$tabs = array(
			'page_setup' => __('Setup',OP_SN),
			'page_thumbnails' => __('Thumbnails',OP_SN),
			'navigation_text' => __('Nav Text',OP_SN),
			'publish_stage' => __('Publish',OP_SN),
			'delete_stage' => __('Delete',OP_SN),
		);
		$counter = 1;
	...
	function localize($array){
		...
		$array['launch_section_title'] = __('Funnel Stage %1$s',OP_SN);
		$this->_page_select();
