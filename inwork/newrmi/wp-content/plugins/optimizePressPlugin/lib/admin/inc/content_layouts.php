...
class OP_Content_Layout_Upgrader extends WP_Upgrader {
	...
	function upgrade_strings() {
		$this->strings['downloading_package'] = __('Downloading update from <span class="code">%s</span>&#8230;', OP_SN);
		$this->strings['unpack_package'] = __('Unpacking the update&#8230;', OP_SN);
		$this->strings['remove_old_failed'] = __('Could not remove the old plugin.', OP_SN);
		$this->strings['remove_old'] = __('Removing old content template files.', OP_SN);
		$this->strings['process_failed'] = __('Content template update failed.', OP_SN);
		$this->strings['process_success'] = __('Content template updated successfully.', OP_SN);
	}

	function install_strings() {
		$this->strings['no_package'] = __('Install content template not available.', OP_SN);
		$this->strings['downloading_package'] = __('Downloading install package from <span class="code">%s</span>&#8230;', OP_SN);
		$this->strings['unpack_package'] = __('Unpacking the package&#8230;', OP_SN);
		$this->strings['installing_package'] = __('Installing the content template&#8230;', OP_SN);
		$this->strings['process_failed'] = __('Content template install failed.', OP_SN);
		$this->strings['process_success'] = __('Content template installed successfully.', OP_SN);
		$this->strings['missing_config'] = __('Content template config file is missing.', OP_SN);
		$this->strings['missing_image'] = __('Content template thumbnail image is missing.', OP_SN);
		$this->strings['remove_old'] = __('Removing old template layout files.', OP_SN);
	}
