...
'.$css.'
    ...
    function print_scripts(){
        ...
        /*$needed = array('editor','word-count','quicktags','wplink','wpdialogs-popup','wp-jquery-ui-dialog','wp-fullscreen','media-upload');
  ...
  google.setOnLoadCallback(function() {
    ...
    function check_nonce($field='_wpnonce',$id='op_liveeditor',$return=false,$func='op_post'){
        ...
        if($return){
            ...
            echo json_encode(array('error'=>__('Verification failed, please refresh the page and try again.',OP_SN)));
            exit;
        }
    }
...
var OP_tinyMCE = '.$init_str.';
    ...
    function delete_layout(){
        ...
        if(!$filename = op_post('filename')){
            exit(json_encode(array('error' => __('Please specify a filename',OP_SN))));
        }
        $filename = stripslashes($filename);
        if(!file_exists($filename) || end(explode('.',$filename)) != 'zip'){
            exit(json_encode(array('error' => __('The filename provided could not be found',OP_SN))));
        }
        @unlink($filename);
...
';
    ...
    function load_content_upload(){
        ...
        if(isset($_REQUEST['_wpnonce'])){
            if(wp_verify_nonce($_REQUEST['_wpnonce'],'op_content_layout_upload')){
                ...
                $data['error'] = __('Verification failed, please refresh and try again.',OP_SN);
            }
        }
    ...
    function get_layout(){
        ...
        $this->check_nonce();
        $this->init_page();
        $out = array('error' => __('Could not find the selected layout, please try again.',OP_SN));
        ...
        foreach($sections as $name => $section){
            if(!(op_page_config('disable',$name) === true)){
                ...
                $this->sections[$name] = array(
                    'object' => $obj->sections(),
                    'title' => $i.'. '.__($section['title'],OP_SN),
