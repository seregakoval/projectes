...
class OptimizePress_Blog_Scripts_Module extends OptimizePress_Modules_Base {
	...
	function meta_box($post){
		...
		<div id="op-meta-scripts">
			<h4>'.__('Other Scripts',OP_SN).'</h4>
			<p class="op-micro-copy">'.__('Add other scripts to your website, this is great for things like Google Analytics',OP_SN).'</p>
			<p class="op-micro-copy">'.__('Code added here will not be rendered to pages in the LiveEditor so you will not see changes until you publish your page and view it live online',OP_SN).'</p>
				...
				$content .= '
					<li>
						<select name="'.$name.'[position][]">
							<option value="header"'.selected('header', $item['position'], false).'>'.__('Header',OP_SN).'</option>
							<option value="footer"'.selected('footer', $item['position'], false).'>'.__('Footer',OP_SN).'</option>
							<option value="css"'.selected('css', $item['position'], false).'>'.__('Custom CSS',OP_SN).'</option>
						</select>
						<textarea name="'.$name.'[script][]" cols="40" rows="5">'.stripslashes(op_attr($item['script'])).'</textarea>
						<div class="op-multirow-controls">
							<a href="#move-up">'.__('Move Up',OP_SN).'</a> | <a href="#move-down">'.__('Move Down',OP_SN).'</a> | <a href="#remove">'.__('Remove',OP_SN).'</a>
						</div>
					</li>';
			endforeach;
			$content .= '
				...
				<div class="clear"></div>
				<a href="#" class="add-new-row">'.__('Add New', OP_SN).'</a>
			</div>
		</div>';
		echo $content;
	}
...
<div class="op-bsw-grey-panel-content op-bsw-grey-panel-no-sidebar">
    <div class="entry-content">
    	<p><?php _e('Add any scripts, code or CSS to this page. If you are using external scripts, ensure you use the Header/Footer dropdown to place them in the correct position in the page code.',OP_SN) ?></p>
        <p><?php _e('Code added here will not be rendered to pages in the LiveEditor so you will not see changes until you publish your page and view it live online.',OP_SN) ?></p>
		<?php 
            ...
            <?php foreach($items as $item): ?>
				<li>
                	<select name="<?php echo $name  ?>[position][]">
                    	<option value="header"<?php selected('header', $item['position']); ?>><?php _e('Header',OP_SN) ?></option>
                    	<option value="footer"<?php selected('footer', $item['position']); ?>><?php _e('Footer',OP_SN) ?></option>
                    	<option value="css"<?php selected('css', $item['position']); ?>><?php _e('Custom CSS',OP_SN) ?></option>
                    </select>
                    ...
                    <div class="op-multirow-controls">
                        <a href="#move-up"><?php _e('Move Up',OP_SN) ?></a> | <a href="#move-down"><?php _e('Move Down',OP_SN) ?></a> | <a href="#remove"><?php _e('Remove',OP_SN) ?></a>
                    </div>
                </li>
			<?php endforeach ?>
            </ul>
	...
	function _load_scripts($type){
		static $scripts = array('header'=>array(),'footer'=>array(),'css'=>array());
		static $checked;
		if(!isset($checked)){
