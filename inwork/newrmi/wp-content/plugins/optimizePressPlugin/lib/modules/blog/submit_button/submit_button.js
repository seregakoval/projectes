;(function($){
	$(document).ready(function(){
		...
		$('.font-dropdown li a').click(function(e){
			...
			$.event.trigger({type: 'update_button_preview', id: parent.attr('id'), owner: parent.closest('.submit-button-container').attr('id'), value: $img.attr('alt'), font_type: $img.attr('data-type'), font_family: $img.attr('data-family')});
		});
		/*
			...
			for (var i in attributes) {
				switch (defaults[i].type) {
					...
					case 'font':
						...
						var $item = $(owner + ' ' + defaults[i].selector + ' img[alt="' + value[0] + '"]');
						/*
