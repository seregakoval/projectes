<?php $url = ($fields['type'] == 'url'); $youtube = ($fields['type'] == 'youtube');?><div class="op-type-switcher-container">
	<select name="<?php echo $fieldname ?>[type]" class="op-type-switcher" id="<?php echo $fieldid ?>type">
    	<option value="embed"<?php echo !$url ? ' selected="selected"':'' ?>><?php _e('Embed code',OP_SN) ?></option>
        <option value="url"<?php echo $url ? ' selected="selected"':'' ?>><?php _e('URL',OP_SN) ?></option>
        <option value="youtube"<?php echo $youtube ? ' selected="selected"':'' ?>><?php _e('YouTube Video',OP_SN) ?></option>
    </select>
    <div class="op-type op-type-embed<?php echo !$file ? ' op-disabled-type' : '' ?>">
    	<label for="<?php echo $fieldid ?>embed"><?php _e('Embed Code:',OP_SN) ?></label>
        <p class="op-micro-copy"><?php _e('Enter external video embed code here.  Enter codes from EVP, or any external hosts like YouTube here',OP_SN) ?></p>
    ...
    <div class="op-type op-type-url<?php echo $file ? ' op-disabled-type' : '' ?>">
    	<label for="<?php echo $fieldid ?>url" class="form-title"><?php _e('URL:', OP_SN) ?></label>
        <p class="op-micro-copy"><?php _e('Enter your hosted video URL here.  We recommend using Amazon S3 or a similar professional video host.',OP_SN) ?></p>
        <input type="text" name="<?php echo $fieldname ?>[url]" id="<?php echo $fieldid ?>url" value="<?php echo $fields['url'] ?>" />

        <label for="<?php echo $fieldid ?>placeholder" class="form-title"><?php _e('Placeholder:',OP_SN) ?></label>
        <p class="op-micro-copy"><?php _e('Select an image to use as a placeholder in your video',OP_SN) ?></p>
        ...
        <div class="op-checkbox-container">
    		...
    		<label for="<?php echo $fieldid ?>hide_controls"><?php _e('Hide Controls:',OP_SN) ?></label>
	        <p class="op-micro-copy"><?php _e('Hide the video controls for your video (control bar etc)',OP_SN) ?></p>
        </div>
        ...
        <div class="op-checkbox-container">
	    	...
	    	<label for="<?php echo $fieldid ?>auto_play"><?php _e('Auto Play:',OP_SN) ?></label>
        	<p class="op-micro-copy"><?php _e('Set your video to auto play on the load of the page',OP_SN) ?></p>
        </div>
        ...
        <div class="op-checkbox-container">
	    	...
	    	<label for="<?php echo $fieldid ?>auto_buffer"><?php _e('Auto Buffering:',OP_SN) ?></label>
        	<p class="op-micro-copy"><?php _e('Enabling auto buffering will allow the video to be loaded before the user clicks play',OP_SN) ?></p>
        </div>
    </div>
    <div class="op-type op-type-youtube<?php echo $file ? ' op-disabled-type' : '' ?>">
    	<label for="<?php echo $fieldid ?>youtube_url" class="form-title"><?php _e('Youtube URL:', OP_SN) ?></label>
        <p class="op-micro-copy"><?php _e('Enter your YouTube video URL here.',OP_SN) ?></p>
        ...
        <div class="op-checkbox-container">
	    	...
	    	<label for="<?php echo $fieldid ?>youtube_auto_play"><?php _e('Auto Play:',OP_SN) ?></label>
        	<p class="op-micro-copy"><?php _e('Set your video to auto play on the load of the page',OP_SN) ?></p>
        </div>
        ...
        <div class="op-checkbox-container">
    		...
    		<label for="<?php echo $fieldid ?>youtube_hide_controls"><?php _e('Hide Controls:',OP_SN) ?></label>
	        <p class="op-micro-copy"><?php _e('Hide the video controls for your video (control bar etc)',OP_SN) ?></p>
        </div>
        ...
        <div class="op-checkbox-container">
	    	...
	    	<label for="<?php echo $fieldid ?>youtube_remove_logo"><?php _e('Hide YouTube logo:',OP_SN) ?></label>
        	<p class="op-micro-copy"><?php _e('Hide YouTube logo in control bar (will still show in corner of video on hover)',OP_SN) ?></p>
        </div>
        <div class="op-checkbox-container">
	    	...
	    	<label for="<?php echo $fieldid ?>youtube_show_title_bar"><?php _e('Show title bar:',OP_SN) ?></label>
        	<p class="op-micro-copy"><?php _e('Show the title bar.',OP_SN) ?></p>
        </div>
        <label for="<?php echo $fieldid ?>youtube_force_hd"><?php _e('Force HD mode:',OP_SN) ?></label>
        <select name="<?php echo $fieldname ?>[youtube_force_hd]" id="<?php echo $fieldid ?>youtube_force_hd">
	    	<option value="none"<?php echo $fields['youtube_force_hd'] == 'none' ? ' selected="selected"':'' ?>><?php _e('None',OP_SN) ?></option>
	        <option value="hd720"<?php echo $fields['youtube_force_hd'] == 'hd720' ? ' selected="selected"':'' ?>><?php _e('720p',OP_SN) ?></option>
	        <option value="hd1080"<?php echo $fields['youtube_force_hd'] == 'hd1080' ? ' selected="selected"':'' ?>><?php _e('1080p',OP_SN) ?></option>
	    </select>
    </div>

    <label for="<?php echo $fieldid ?>dimensions" class="form-title"><?php _e('Dimensions:', OP_SN) ?></label>
    <p class="op-micro-copy"><?php _e('Set the dimensions of your video',OP_SN) ?></p>
	...
	<label for="<?php echo $fieldid ?>width" class="form-title"><?php _e('Width:', OP_SN) ?></label>
	<p class="op-micro-copy"><?php _e('Set the width of your video',OP_SN) ?></p>
	...
	<label for="<?php echo $fieldid ?>height" class="form-title"><?php _e('Height:', OP_SN) ?></label>
	<p class="op-micro-copy"><?php _e('Set the height of your video',OP_SN) ?></p>
    ...
    <label for="<?php echo $fieldid ?>margin_top" class="form-title"><?php _e('Top Margin:', OP_SN) ?></label>
    <p class="op-micro-copy"><?php _e('Set the top margin of your video',OP_SN) ?></p>
    <input type="text" name="<?php echo $fieldname ?>[margin_top]" id="<?php echo $fieldid ?>margin_top" value="<?php echo empty($fields['margin_top']) ? '0' : $fields['margin_top']; ?>" />
  	...
  	<label for="<?php echo $fieldid ?>margin_bottom" class="form-title"><?php _e('Bottom Margin:', OP_SN) ?></label>
    <p class="op-micro-copy"><?php _e('Set the bottom margin of your video',OP_SN) ?></p>
