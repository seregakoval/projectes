...
class OptimizePress_Blog_Comments_Module extends OptimizePress_Modules_Base {
    ...
    function meta_box($post){
        ...
        <div id="op-meta-comments">
            <label for="'.$id.'wordpress_enabled">'.__('WordPress Comments',OP_SN).'</label>
            <p class="op-micro-copy">'.__('Let your visitors comment on this content via the Wordpress comments system',OP_SN).'</p>
            ...
            <label for="'.$id.'facebook_enabled">'.__('Facebook Comments',OP_SN).'</label>
            <p class="op-micro-copy">'.__('Let your visitors comment on this content via the Facebook comments system. Ensure you have setup your Facebook App ID in the OptimizePress Blog Settings',OP_SN).'</p>
...
<div class="op-bsw-grey-panel-content op-bsw-grey-panel-no-sidebar">
    <div class="op-bsw-grey-panel">
        <div class="op-bsw-grey-panel-header cf">
            <h3><?php _e('WordPress Comments',OP_SN) ?></h3>
    ...
    <div class="op-bsw-grey-panel">
        <div class="op-bsw-grey-panel-header cf">
            <h3><a href="#"><?php _e('Facebook Comments',OP_SN) ?></a></h3>
            <div class="op-bsw-panel-controls cf">
        ...
        <div class="op-bsw-grey-panel-content op-bsw-grey-panel-no-sidebar">
            ...
            <label for="op_<?php echo $section_name ?>_facebook_title" class="form-title"><?php _e('Title', OP_SN) ?></label>
            <p class="op-micro-copy"><?php _e('Enter the title for your Facebook comments section',OP_SN) ?></p>
            ...
            <div class="op-checkbox-container">
                ...
                <label for="op_<?php echo $section_name ?>_facebook_hide_like"><?php _e('Hide Like button', OP_SN) ?></label>
                <p class="op-micro-copy"><?php _e('Hide the Facebook Like button above the comments',OP_SN) ?></p>
            </div>

            <label for="op_<?php echo $section_name ?>_facebook_language" class="form-title"><?php _e('Language', OP_SN) ?></label>
            <p class="op-micro-copy"><?php _e('Set the language for your Facebook comments system here',OP_SN) ?></p>
            ...
            <div class="op-checkbox-container">
                ...
                <label for="op_<?php echo $section_name ?>_facebook_dark_site"><?php _e('Dark Site?', OP_SN) ?></label>
                <p class="op-micro-copy"><?php _e('Check if your site is dark to allow easier to see colours', OP_SN) ?></p>
            </div>

            <label for="op_<?php echo $section_name ?>_facebook_posts_number" class="form-title"><?php _e('Number of posts to show', OP_SN) ?></label>
            <p class="op-micro-copy"><?php _e('Enter the number of Facebook comments to show by default_attr(we recommend 10)',OP_SN) ?></p>
            ...
            <?php if($page): ?>
            <label for="op_<?php echo $section_name ?>_facebook_src_url" class="form-title"><?php _e('Facebook Comments Source URL', OP_SN) ?></label>
            <p class="op-micro-copy"><?php _e('If you would like to associate all your comments with a particular page on your site, or show the same comments on multiple pages (perhaps for a launch) enter the URL here. We recommend using your root domain URL.',OP_SN) ?></p>
    ...
    function post_meta($array){
        ...
        $args = array(
            __('<p class="post-meta"><a href="%1$s" title="%2$s" rel="author">%3$s</a></p>', OP_SN),
    ...
    function output_comments($comments=array(),$return=false,$settings=array()){
        ...
        $arr = array('comments','facebook');
        $checks = array('id','language','hide_like','dark_site','posts_number','width','title','src_url');
        $pb = defined('OP_PAGEBUILDER');
        foreach($checks as $chk){
        ...
        $out = '
<div id="fb-root"></div>
<div id="fbComments"'.$class.'>
'.urldecode($title).
($hide_like != 'Y' ? '
    <fb:like href="'.$src_url.'" send="false"'.$widthstr.' show_faces="false"'.$color_scheme.' font="arial" class="op-fbComments--like"></fb:like>':'').'
    <fb:comments href="'.$src_url.'"'.$color_scheme.$widthstr.' num_posts="'.$posts_number.'" publish_feed="true" migrated="1" class="op-fbComments--comments"></fb:comments>
</div>';
// xid="'.$xid.'"
