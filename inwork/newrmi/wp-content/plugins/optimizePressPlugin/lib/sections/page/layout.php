...
class OptimizePress_Sections_Layout {
	...
	function sections(){
		...
		if(!isset($sections)){
			...
			$vals = array(__('Header &amp; Navigation', OP_SN),__('Feature Area', OP_SN),__('Feature Title', OP_SN),__('Footer Area', OP_SN));
			$type = op_page_option('theme','type');
			if($type == 'launch'){
				array_splice($keys,1,2,array('feature_area','launch_nav'));
				array_splice($vals,1,2,array(__('Feature Area', OP_SN),__('Launch Funnel Menu', OP_SN)));
			} elseif($type == 'landing'){
				unset($keys[1],$keys[2],$vals[1],$vals[2]);
				array_splice($keys,0,1,array('header_layout', 'landing_bg', 'size_color'));
				array_splice($vals,0,1,array(__('Header &amp; Navigation', OP_SN), __('Landing Page Background', OP_SN), __('Size &amp; Colour', OP_SN)));
			}
	...
	function save_header_layout($op){
		...
		$fields = array('logo','bgimg','repeatbgimg','bgcolor','link_color', 'disable_link', 'header_link');
		$has_error = false;
