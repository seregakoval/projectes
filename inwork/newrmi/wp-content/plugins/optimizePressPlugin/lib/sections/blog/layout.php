...
class OptimizePress_Sections_Layout {
			...
			if(isset($layouts['layouts'][$new_layout['option']])){
				...
				foreach($def_widths as $name => $options){
					...
					if(!($val >= $options['min'] && $val <= $options['max'])){
						op_tpl_error('op_sections_column_layout_withs_'.$name,sprintf(__('The %1$s column must be between %2$s and %3$s.',OP_SN),$options['title'],$options['min'],$options['max']),'layout_column_layout');
					} else {
	...
	function color_fields(){
		$color_fields = array(
			'footer_bg' => __('Footer background colour',OP_SN),
			'topbar_bg' => __('Topbar BG Colour',OP_SN),
			'link_color' => __('Link Colour',OP_SN),
		);
