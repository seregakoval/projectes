...
class OP_TabbedPostInfoWidget extends WP_Widget {
	//Constructor
	function OP_TabbedPostInfoWidget(){
	parent::WP_Widget(false, __('OptimizePress: Recent Post Tabs', OP_SN), array('description' => __('Displays tabbed lists of popular posts, recent posts, comments and tags from your blog', OP_SN)));
	}
	...
	<p>
			<label for="<?php echo $this->get_field_id('popular')?>">
				<?php _e('Show Popular Posts:', OP_SN); ?> <input id="<?php echo $this->get_field_id('popular')?>" name="<?php echo $this->get_field_name('popular')?>" type="checkbox" value="1"<?php echo ($popularPosts==1 ? ' checked' : '')?> />
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('recent')?>">
				<?php _e('Show Recent Posts:', OP_SN); ?> <input id="<?php echo $this->get_field_id('recent')?>" name="<?php echo $this->get_field_name('recent')?>" type="checkbox" value="1"<?php echo ($recentPosts==1 ? ' checked' : '')?> />
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('comments')?>">
				<?php _e('Show Post Comments:', OP_SN); ?> <input id="<?php echo $this->get_field_id('comments')?>" name="<?php echo $this->get_field_name('comments')?>" type="checkbox" value="1"<?php echo ($postComments==1 ? ' checked' : '')?> />
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('tags')?>">
				<?php _e('Show Post Tags:', OP_SN); ?> <input id="<?php echo $this->get_field_id('tags')?>" name="<?php echo $this->get_field_name('tags')?>" type="checkbox" value="1"<?php echo ($postTags==1 ? ' checked' : '')?> />
			</label>
		</p>
		<?php
	}
	...
	function widget($args, $instance){
		...
		//Set tabs to show
		$popularTab = (isset($instance['popular']) && $instance['popular']==1 ? array('popular' => array(__('Popular',OP_SN),'theme'.$theme.'_popular_posts',array(array('ulclass'=>'tab-popular miniposts article-list')))) : array());
		$recentTab = (isset($instance['recent']) && $instance['recent']==1 ? array('recent' => array(__('Recent',OP_SN),'theme'.$theme.'_recent_posts',array(array('ulclass'=>'tab-recent miniposts article-list')))) : array());
		$commentsTab = (isset($instance['comments']) && $instance['comments']==1 ? array('comments' => array(__('Comments',OP_SN),'theme'.$theme.'_recent_comments',array(array('ulclass'=>'tab-comments miniposts article-list')))) : array());
		$tagsTab = (isset($instance['tags']) && $instance['tags']==1 ? array('tags' => array(__('Tags',OP_SN),'theme'.$theme.'_list_tags',array(array('ulclass'=>'tab-tags miniposts article-list')))) : array());
