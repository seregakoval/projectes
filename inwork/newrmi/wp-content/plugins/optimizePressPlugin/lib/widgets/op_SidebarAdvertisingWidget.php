...
class OP_SidebarAdvertisingWidget extends WP_Widget {
    //Constructor
    function OP_SidebarAdvertisingWidget(){
	parent::WP_Widget(false, __('OptimizePress: Sidebar Advertising Block', OP_SN), array('description' => sprintf(__('Displays ad blocks in the sidebar. Can be enabled and customized from the <a href="%s">Blog Settings > Modules > Sidebar Advertising</a> section.', OP_SN), admin_url('admin.php?page=optimizepress-theme-settings#modules'))));
    }

    //Options form for admin section
    function form($instance){
	?>
	<p><?php printf(__('To use this widget you must enable the Sidebar Advertising Module from the <a href="%s">Blog Settings > Modules</a> section of OptimizePress', OP_SN), admin_url('admin.php?page=optimizepress-theme-settings#modules')); ?></p>
	<?php
    }
