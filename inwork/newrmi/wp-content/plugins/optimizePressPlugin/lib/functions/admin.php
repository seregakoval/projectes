...
function op_tiny_mce( $content, $editor_id, $settings = array() ) {
    ...
    if (!isset($wysiwygs)) $wysiwygs = function_exists('wp_editor');
    if($wysiwygs){
...
function op_upload_field($fieldid,$value='',$return=false,$file_or_url='url',$disable_url=false){
    ...
    <div class="op-file-uploader">
        <a href="media-upload.php?post_id=0&amp;op_uploader=true'.($disable_url?'&amp;op_uploader_url_disable=Y':'').'&amp;TB_iframe=1" class="thickbox button" title="'.esc_attr__( 'Add Media' ).'" style="background-image:url(images/white-grad.png)">'.__('Select File',OP_SN).'</a>';
        ...
        if($value != '' && $value !== false){
            ...
            if(in_array($ext,array('jpg', 'jpeg', 'gif', 'png'))){
                $html .= '<a href="'.$value.'" target="_blank" class="preview-image"><img src="'.$value.'" alt="uploaded-image" /></a><a href="#remove" class="remove-file">'.__('Remove Image',OP_SN).'</a>';
            } else {
                $html .= '<a href="'.$value.'" target="_blank" class="preview-image">'.__('View File',OP_SN).'</a><a href="#remove" class="remove-file">'.__('Remove File',OP_SN).'</a>';
            }
        }
        $html .= '
            </div>
...
function op_default_link($fieldid,$value,$return=false){
    if($value){
        $default = '
            <a href="#'.$fieldid.'" class="default-val-link">'.__('Default',OP_SN).'</a>
...
function op_asset_help_vid($sections,$return=false){
    ...
    return op_help_url($sections,'<a href="{url}" tabindex="-1" title="'.__('Help',OP_SN).'" class="help-me"><span>?</span></a>{vid}',$return);
}
function op_help_vid($sections,$return=false){
    return op_help_url($sections,'<a href="{url}" title="'.__('Help',OP_SN).'" class="help-video op-help-vid tooltip animated pulse"><img src="'.op_img('',true).'help-video.png" alt="Help" width="22" height="22" /></a>{vid}',$return);
}
...
function _op_notification($msg='', $type = 0, $hide_button = false, $echo = true, $extra_class = '', $element_id = ''){
    ...
    $notification = '<div class="op-notify ' . $className . $extra_class . '" ' . $id . '><img src="'.op_img('notify-'.$className.'.png',true).'" alt="'.ucfirst($className).'"/><span><strong>'.ucfirst($className).'! </strong>'.__($msg,OP_SN).'</span>'.($hide_button?'':'<div class="op-notify-close"></div>').'</div>';
