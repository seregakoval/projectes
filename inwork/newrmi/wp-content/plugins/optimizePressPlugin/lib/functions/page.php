...
function op_page_header($color=''){
	...
	if($alongside && $logoimg = op_page_option('header_layout','logo')){
		if (empty($disable_link) && empty($header_link)) {
			...
			if ($disable_link === 'on') {
				$logo = '<h1 class="op-logo"><img src="'.$logoimg.'" alt="'.$alt.'" /></h1>';
			} else {
				$logo = '<h1 class="op-logo"><a href="'.esc_url( $header_link ).'"'.$title_str.' rel="home"><img src="'.$logoimg.'" alt="'.$alt.'" /></a></h1>';
			}
		}
		...
		if (empty($disable_link) && empty($header_link)) {
			...
			if ($disable_link === 'on') {
				$logo = '<h1 class="banner-logo"><img src="'.$bannerimg.'" alt="'.$alt.'" /></h1>';
			} else {
				$logo = '<h1 class="banner-logo"><a href="'.esc_url( $header_link ).'"'.$title_str.' rel="home"><img src="'.$bannerimg.'" alt="'.$alt.'" /></a></h1>';
			}
		}
		...
		if (empty($disable_link) && empty($header_link)) {
			$logo = '<div class="site-logo"><h1 class="site-title"><a href="'.esc_url( home_url( '/' ) ).'"'.$title_str.' rel="home">'.$title.'</a></h1><h2 class="site-description">'.$slogan.'</h2></div>';
		} else {
			if ($disable_link === 'on') {
				$logo = '<div class="site-logo"><h1 class="site-title">'.$title.'</h1><h2 class="site-description">'.$slogan.'</h2></div>';
			} else {
				$logo = '<div class="site-logo"><h1 class="site-title"><a href="'.esc_url( $header_link ).'"'.$title_str.' rel="home">'.$title.'</a></h1><h2 class="site-description">'.$slogan.'</h2></div>';
			}
		}
	}
			...
			<div class="fixed-width cf">
				...
				<div class="sixteen columns"><?php
					...
					<nav class="navigation fly-to-left">
						<ul id="navigation-alongside">'.wp_nav_menu( array( 'menu' => $nav['nav'], 'walker' => new Op_Arrow_Walker_Nav_Menu(), 'items_wrap' => '%3$s', 'container' => false, 'echo'=>false ) ).'
						</ul>
					</nav>';
					?>
				</div>
...
function op_page_advanced_color_scheme($css){
	...
	$options = array(
		'page' => array(
			...
			array('color', 'a, a:visited', "['link_color']['color']"),
		...
		'nav_bar_alongside' => array(
			array('background-color','body .container div.include-nav .navigation ul#navigation-alongside li:hover > a','nav_bar_bg_nav_hover'),
			array('background-color','body .container div.include-nav .navigation ul#navigation-alongside li ul.sub-menu li > a','nav_bar_bg'),
			array('background-color','body .container div.include-nav .navigation ul#navigation-alongside li ul.sub-menu li:hover > a','nav_bar_bg_hover'),
		...
		'nav_bar_above' => array(
			...
			array('gradient','.nav-bar-above ul li:hover','nav_bar_hover_start','nav_bar_hover_end','is_gradient'=>false),
		...
		'nav_bar_below' => array(
			...
			array('gradient','.nav-bar-below ul li:hover','nav_bar_hover_start','nav_bar_hover_end','is_gradient'=>false),
...
function op_page_typography_elements($elements){
	$new_elements = array(
		'footer_link_color' => array(
			'name' => __('Footer Link Text Colour',OP_SN),
			'help' => __('Choose the hyperlink text colour for your page footer area',OP_SN),
			'text_decoration' => true,
		),
		'footer_link_hover_color' => array(
			'name' => __('Footer Link Hover Text Colour',OP_SN),
			'help' => __('Choose the hyperlink hover text colour for your page footer area',OP_SN),
			'text_decoration' => true,
		),
		'feature_text_color' => array(
			'name' => __('Feature Area Text Colour',OP_SN),
			'help' => __('Choose the text colour for the feature area content',OP_SN),
		),
		'feature_link_color' => array(
			'name' => __('Feature Area Link Colour',OP_SN),
			'help' => __('Choose the hyperlink colour for the feature area content',OP_SN),
			'text_decoration' => true,
		),
		'feature_link_hover_color' => array(
			'name' => __('Feature Area Link Hover Colour',OP_SN),
			'help' => __('Choose the hyperlink hover colour for the feature area content',OP_SN),
			'text_decoration' => true,
		)
	);
	$elements['color_elements'] = array_merge($elements['color_elements'],$new_elements);
