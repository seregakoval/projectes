...
<script type="text/javascript">
  ...
  google.setOnLoadCallback(function() {
	...
	function font_visual_dropdown($id='', $value = null){
		$font_types = $this->all_fonts_array();
		$str = '<ul class="cf"><li><a href="#">'.__(OP_STRING_FONT_THEME_DEFAULT, OP_SN).'</a></li></ul>';
		$path = OP_IMG.'fonts/';
		$selected = __('Default',OP_SN);
		foreach($font_types as $type => $options){
	...
	function font_style_dropdown($name,$value='',$id='',$extra=''){
		...
		if(!isset($types)){
			$types = array(
				'' => __(OP_STRING_FONT_STYLE,OP_SN),
				'300' => __('Thin',OP_SN),
				'normal' => __('Normal',OP_SN),
				'italic' => __('Italic',OP_SN),
				'bold' => __('Bold',OP_SN),
				'bold italic' => __('Bold/Italic',OP_SN),
			);
		}
