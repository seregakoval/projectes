...
class OptimizePress_Page_Feature_Area_Base {
	...
	function _load_style($name,$options,$feature_settings,$fonts){
		...
		if(defined('OP_LIVEEDITOR')){
			$start = '<div class="op-feature-area" data-unload="0"><div class="op-feature-settings"><a href="#{link_id}" class="feature-settings">' . __('Settings', OP_SN) . '</a></div>';
			$end = '</div>';
		}
	...
	function update_feature(){
		...
		$output['output'] = '<div class="op-feature-area" data-unload="0">'.(defined('OP_LIVEEDITOR')?str_replace('{link_id}','feature_area_'.$option,'<div class="op-feature-settings"><a href="#{link_id}" class="feature-settings">' . __('Settings', OP_SN) . '</a></div>'):'').$output['output'].op_get_var($options,'after_element').'</div>';
		return $output;	
	}
		...
		foreach($settings as $name => $options){
			...
			<form action="#" id="feature_area_'.$name.'" class="op-feature-area">
				...
				<div class="op-insert-button cf">
					<button type="submit" class="editor-button"><span>'.__('Update Feature',OP_SN).'</span></button>
				</div>
			</form>';
			}
		}
