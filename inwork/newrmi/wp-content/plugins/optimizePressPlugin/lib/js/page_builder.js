;(function($){
	...
	var methods = {
		'step_1': function(){
			...
			var form = this,
				c = $('#op_ajax_checker'), w = c.find('img'), check = c.find('a[href$="#check"]'),
				cancel = c.find('a[href$="#cancel"]'), xhr,
				err = c.find('span.error'), success = c.find('span.success');
					...
					xhr = $.ajax(OptimizePress.ajaxurl,{
						...
						'success': function(resp){
							...
							if(resp.valid === true){
								...
								valid_slug = true;
								if(submit_form){
									window.parent.OP.reload_page = true;
									form.trigger('submit');
								} else {
									$submit_button.removeClass('op-loading');
								}
