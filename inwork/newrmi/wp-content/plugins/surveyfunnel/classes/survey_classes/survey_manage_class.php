...
/**
	...
	function initSurveyManage() {
		...
		switch ($this->action) {
            case 'RESET_FUNNEL':
                ...
                answer_images = NULL ,
                answer_flows = NULL ,
                use_shortcode = NULL WHERE wp_sf_surveys.survey_id =$this->survey_id;
            ";
            $wpdb->query($query);
		echo 1;
		die();
	...
	// BEGIN Added By Arvind On 7-AUG-2013  For Add UserInformation
			...
			if($r-> question_type ==3){
				...
				<div class=\"toolbar\">
				<a href=\"javascript:void(0);\" onclick=\"removeSFQuestion('$tmpID');\"><img src=\"" . plugins_url() . "/".SF_PLUGIN_DIR."/images/btn/delete_btn.png\" border=\"0\"></a>
				</div>
			...
			else
		  {
			$header=htmlspecialchars_decode($this->default_question_header, ENT_QUOTES);
		  	...
		  	echo "<li id=\"li_$tmpID\" class=\"sf_que_li\">
					<div class=\"sfQuestionDiv\">
						<div class=\"toolbar\">
							<a href=\"javascript:void(0);\" onclick=\"$jsLink('$tmpID', '" . SF_PLUGIN_URL . "');\"><img src=\"" . SF_PLUGIN_URL."/images/btn/edit_btn.png\" border=\"0\"></a>
							<a href=\"javascript:void(0);\" onclick=\"removeSFQuestion('$tmpID');\"><img src=\"" . SF_PLUGIN_URL ."/images/btn/delete_btn.png\" border=\"0\"></a>
						</div>
						<div class=\"display\"></div>
						<div class=\"sf_preview_container\" style=\"\">
							<div class=\"sf_preview_header\">$header</div>
							<div class=\"$isContent\" style\"\">$display</div>";
		  }
			...
			if ($r->question_type == 1 || $r->question_type==3) // Change the condition by Arvind for User Information
			{
				...
				foreach ($tmpAnswers as $answer_index => $answer_display) {
					if (trim($answer_display)) {
						...
						echo "<div class=\"answerRule\"><select name=\"answer_rules[$tmpID][]\" onmousedown=\"updateSFFlows(this, 1);\" class=\"sfRuleDropDown\"><option value=\"\"></option>";
...
$wpdb->insert($wpdb->prefix . 'sf_survey_rules', $ruu);
	...
	private function deleteSurvey() {
		...
		$wpdb->update($wpdb->prefix . 'sf_surveys', 
							...
							array('survey_id' => $this->survey_id));
	/*		$wpdb->query(
	"DELETE FROM ".$wpdb->prefix . 'sf_surveys'." 	WHERE survey_id = '13' 	";
);	*/
