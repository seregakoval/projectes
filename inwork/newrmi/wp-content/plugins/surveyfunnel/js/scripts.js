...
function performSFQuestionAdd(iID, iURL) {
	...
	var tmpHTML = '<li id="li_' + iID + '" class="sf_que_li">\
		<div class="sfQuestionDiv">\
			<div class="toolbar">\
				<a href="#" onclick="editSFQuestion(\'' + iID + '\', \'' + iURL + '\');return false;\"><img src="' + iURL + '/images/btn/edit_btn.png" border="0"></a>\
				<a href="#" onclick="removeSFQuestion(\'' + iID + '\');return false;\"><img src="' + iURL + '/images/btn/delete_btn.png" border="0"></a>\
			</div>\
	...
	jQuery.each(tmpAnswers, function() {
		if (this != '') {
			...
			tmpHTML = tmpHTML + '<div class="answerRule"><select name="answer_rules[' + iID + '][]" onmousedown="updateSFFlows(this, 1);" class="sfRuleDropDown"><option value=""></option></select></div>';
...
function performSFContentAdd(iID, iURL) {
	...
	var tmpHTML = '<li id="li_' + iID + '" class="sf_que_li">\
		<div class="sfQuestionDiv">\
			<div class="toolbar">\
				<a href="#" onclick="editSFContent(\'' + iID + '\', \'' + iURL + '\');return false;\"><img src="' + iURL + '/images/btn/edit_btn.png" border="0"></a>\
				<a href="#" onclick="removeSFQuestion(\'' + iID + '\');return false;\"><img src="' + iURL + '/images/btn/delete_btn.png" border="0"></a>\
			</div>\
			<div class="display"></div>\
                        <div class="sf_preview_container" style="">\
...
function copySFFunnel(iID, iURL) {
	var tmpName = prompt('Please specify the new survey name');
...
function deleteSFFunnel(iID, iURL) {
	...
	var answer = confirm("Are you sure you want to delete this funnel?")
	if (answer) {	
	...
	*/
	jQuery('#checkbox_value').change(function(){
		...
		if( jQuery('#checkbox_value').is(':checked') ){
			jQuery('#text-field').show();
		}
		else{
			jQuery('#text-field').val('');
			jQuery('#text-field').hide();
		}
