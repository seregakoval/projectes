...
function editslider($id)
  {
	  ...
	  global $wpdb;
	   ...
	   if(isset($_GET["removeslide"])){
		   $getremoveslide = intval($_GET["removeslide"]);
	       if($getremoveslide != 0){
...
function apply_cat($id)
{
 ...
 if($_SESSION['csrf_token_hugeit_1752'] == $_REQUEST['csrf_token_hugeit_1752']){
	...
	$script_cat = preg_replace('#<script(.*?)>(.*?)</script>#is', '', stripslashes($_POST["content"]));
         ...
         $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_itslider_sliders SET  name = '%s'  WHERE ID = %d ", $post_slname, $id));
	 ...
	 $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_itslider_sliders SET  slider_list_effects_s = '%s'  WHERE ID = %d ", $post_slider_effects_list, $id));
		 ...
		 $wpdb->query("UPDATE ".$wpdb->prefix."huge_itslider_sliders SET  video_autoplay = 'off'");     /*dobavka*/  
		 $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_itslider_sliders SET  video_autoplay = '%s'  WHERE ID = %d", $post_show_video_autoplay , $id));    /*dobavka*/ 
                 $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_itslider_sliders SET  random_images = '%s'  WHERE ID = %d", $_POST["random_images"], $id));    /*dobavka*/ 
...
//      $wpdb->query($sql_2);
	...
	if(isset($_POST["posthuge-it-description-length"])){
		$post_huge_it_description = intval($_POST["posthuge-it-description-length"]);
	 $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_itslider_sliders SET  published = %d WHERE id = %d ", $post_huge_it_description, intval($_GET['id'])));
}
	?>
