 ...
 * @author Max Bond
 * 
 * Includes table location filter class
 *
 */
class q2w3_table_location_filter extends _q2w3_table_filter {
	
	protected $inc_obj;
	
	protected $filters;
	
	const VAR_NAME = 'location';
	...
	public function controls() {
		...
		$count_filters = $wpdb->get_results('SELECT count(DISTINCT '. $inc_obj->location->col_name .') FROM '. $inc_obj->table(), ARRAY_N); 
		...
		$filters += $inc_obj->location->input_values;
		...
		foreach ($filters as $filter_key=>$filter_name) { // sub filter cycle
			...
			$condition = 'WHERE '. $inc_obj->location->col_name .' = '. $filter_key;
		...
		if ($res) {
			...
			$ul = '<div class="subsubsub" style="margin-top: 0;">'.PHP_EOL;
	...
	public function sql() {
		...
		return $this->inc_obj->location->col_name .' = '.$cur_filter;
