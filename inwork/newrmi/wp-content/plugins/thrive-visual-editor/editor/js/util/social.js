...
(function ($) {
    ...
    TVE_Content_Builder.social = {
        ...
        add_class: function ($btn, $element) {
            var $socialElement = $element.find('.tve_social_custom');
            var new_class = $btn.attr('data-cls') ? $btn.attr('data-cls') : $btn.attr('id');
            if ($btn.attr('data-layout')) {
                $socialElement.removeClass('tve_social_ib tve_social_itb tve_social_cb');
            } else { // button style
                $socialElement.removeClass('tve_style_1 tve_style_2 tve_style_3 tve_style_4 tve_style_5');
            }
        ...
        save: function () {
            var config = this.readLightboxConfig(),
                errors = $social.getValidationErrors(config);
        ...
        shareCount: function ($input) {
            ...
            $social.setConfig(config);
            $social.totalShareCount();
        },
        /**
        ...
        enableSortable: function ($btn, $element, event) {
            ...
            $element.find(".tve_social_items").sortable({
                ...
                update: function (event, ui) {
                     ...
                     */
                    if ($social.type == 'default' && ui.item.hasClass('tve_s_xing_share')) {
                        var $item = $($social.getNetworkHtml('xing_share'));
                        ui.item.after($item);
                        ui.item.remove();
        ...
        openMedia: function ($btn) {
            ...
            thrive_open_media(null, 'load', 'simple_image', $target, function (attachment) {
                $btn.parent().find('input.tve-image-value').val(attachment.url);
